...
class KurokoRewards(RewardsCfg):
    ...
    flat_toe_penalty = RewTerm(
        ...
        weight=5.0,
        params={
            'asset_cfg': SceneEntityCfg('robot', body_names=['ankle_r_yaw_link', 'ankle_l_yaw_link']),
            'margin': 0.0,
            'gain': 1.0,
        },
    )
    ...
    joint_deviation_elbows = RewTerm(
        ...
        params={
            'asset_cfg':
                SceneEntityCfg('robot', joint_names=['elbow_l_front', 'elbow_l_rear', 'elbow_r_front', 'elbow_r_rear'])
        },
    )
