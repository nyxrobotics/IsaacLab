...
def step_reflex_penalty(
    ...
    weight_shift_penalty_scale: float = 0.0,
    body_offset_forward: float = 0.0,
    weight_shift_angle_margin: float = 1.57,
) -> torch.Tensor:
    """
    Step-reflex penalty.
    ...
    # -------------------------------------------------------
    # 4-2) Weight-shift penalty (torso velocity toward shorter-contact foot)
    #
    # 両足接地中:
    ...
    # torso → foot vectors (world XY)
    vec_L = left_foot_pos[:, :2] - torso_pos[:, :2]
    vec_R = right_foot_pos[:, :2] - torso_pos[:, :2]
    ...
    # torso_dir_xy と desired_dir_xy の角度
    dot_vel = torch.sum(torso_dir_xy * desired_dir_xy, dim=1).clamp(-1.0, 1.0)
    angle = torch.acos(dot_vel)             # [0, π]

    # 許容マージン（デフォルト: 1.57 rad = 90°）を超えた部分だけ penalize
    angle_excess = torch.clamp(angle - weight_shift_angle_margin, min=0.0)

    # penalty = 速度 × 角度超過 × scale
    raw_penalty = torso_speed_xy * angle_excess * weight_shift_penalty_scale
    weight_shift_penalty = -raw_penalty * double_is_contact

    # unstable 時は一律ペナルティ
    # weight_shift_penalty = torch.where(
    #     (~is_stable_mask),
    #     -vel_penalty_scale * torch.ones_like(weight_shift_penalty) * 100.0,
